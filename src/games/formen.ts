// Shapes, drawn flat, for Das Haus der Formen.
//
// Lifted from Lernkiste unchanged except for the silhouette at the
// bottom, which this game needs and that one did not.
//
...
const FARBE: Record<Form, readonly string[]> = {
  ...
  quadrat: P.citrus,
  rechteck: P.blossom,
  // NOT `glow`, which is the colour of a Mathe-Stern.
  //
  // The first screenshot of this house had a gold star sitting among
  // the answer cards of a game whose entire reward is a gold star, with
  // another one counting up in the corner of the same screen. A
  // six-year-old reading "the gold star is the good one" would be
  // reading the rest of this game correctly and this card wrongly, and
  // that is the game's fault, not theirs.
  stern: P.plum,
  herz: P.fruit,
};
...
/**
 * The same shape, flat grey, as the QUESTION.
 *
 * Lernkiste asked this question with a voice and nothing else: an empty
 * stage, a speaker button, and four coloured cards. That is a broken
 * exercise here, because rule 15 says the sound is off-switchable in
 * two taps and this game gets played in waiting rooms — with the sound
 * off, a child would see four shapes and no question at all.
 *
 * So the question is SHOWN, in grey and at a different size from the
 * cards, and the name is spoken over the top of it. Without the sound
 * it is still a whole exercise; with it, the word arrives attached to
 * the thing, which is how the word is learned in the first place.
 *
 * Grey and resized on purpose. Matching a green triangle to an
 * identical green triangle is a colour-matching exercise a two-year-old
 * can do. Matching a grey silhouette to a green triangle is recognising
 * the FORM apart from its colour and its size, which is the actual
 * skill and is worth a house.
 */
export function silhouette(f: Form, size = S): Px {
  const p = form(f, size);
  // Every colour the shape used becomes one flat grey — except the ink
  // edge, which stays, so the outline still reads on the pale stage.
  // `remap` works on the buffer in place, which is why this starts from
  // its own `form()` rather than from a shared one.
  const grau = shade(P.wool, 1);
  p.remap((c) => (c === INK ? INK : grau));
  return p;
}

/** A canvas of the silhouette, for the question stage. */
export function silhouetteCanvas(f: Form, scale: number): HTMLCanvasElement {
  const src = silhouette(f).toCanvas();
  const c = document.createElement('canvas');
  c.width = src.width * scale;
  c.height = src.height * scale;
  const ctx = c.getContext('2d', { willReadFrequently: true })!;
